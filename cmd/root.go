...
func NewRootCommand() (*cobra.Command, *RootOptions) {
	...
	cmd := &cobra.Command{
		Use:   "climos-exporter",
		Short: "Exports climos values to Prometheus",
		Long:  ``,
	}

	cmd.PersistentFlags().StringVar(&rootOptions.configFile, "config", "", "config file (default is $HOME/.climos-exporter.yaml)")
...
func (o *RootOptions) initConfig() {
	if o.configFile != "" {
		...
		// Search config in home directory with name ".climos-exporter" (without extension).
