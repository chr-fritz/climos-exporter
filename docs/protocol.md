...
Devices on the bus: master `ETA0036E31E`, fan controller `SWZ0025B27A`,
...
an article number while doing so: `0x0104` returns `0x0d` (`ETA0036E31E`),
`0x0108` returns `0x19` (`SWZ0025B27A`), `0x0109` returns `0x1c`
(`ST00064E26E`). The remaining addresses are asked twice and stay silent. The
address map in the `Adressen` sheet therefore does not apply to this firmware:
it assigns `0x0105` and `0x0106` to the defroster and the electric heater, and
...
- **1 and 2 bytes** signed integer, little endian.
...
| `0x09` |    16 | empty name slot                  | 60 s   | `00` + 15 spaces | measured   |
...
| ------ | ---------------------------------------------- | --------------: | -------------: | ------------------------- |
| `0x26` | total operating time                           | 3 y 342 d 21:04 | 4 y 58 d 06:06 | +1 min per running minute |
| `0x85` | second counter, fixed 20 d 13:37 behind `0x26` | 3 y 322 d 07:27 | 4 y 37 d 16:29 | as above                  |
| `0x3e` | time left until the filter change              |     145 d 17:34 |     63 d 08:34 | −1 day per day            |
| `0x3d` | counter, only in the restart dump              |     160 d 00:00 |    160 d 00:00 | —                         |
| `0x3f` | counter, only in the restart dump              |      11 d 00:00 |     11 d 00:00 | —                         |

The filter interval sits in `0x3a` = 100 days and matches the span `0x3e` counts
down over. Both operating counters run only while the unit has power, so the
difference to wall clock time is accumulated downtime.

### Identity, only during the startup scan

| Reg            | Width | Value                   | Meaning                                    |
| -------------- | ----: | ----------------------- | ------------------------------------------ |
| `0x0d`         |    11 | `ETA0036E31E`           | article number, master                     |
| `0x19`         |    11 | `SWZ0025B27A`           | article number                             |
| `0x1c`         |    11 | `ST00064E26E`           | article number                             |
...
| `0x34` – `0x3a` | 17, 25, 38, 47, 56, 64, 100 | the seven step ladder the sheet attributes to an LED panel |
...
It is installed, it simply never ran during any of the recordings. In the power
...
is fully explained by the fan. That is why the preheater appears in no recording
and why the register map holds no register for it.

When it next comes on it will most likely report through a register that is not
in the table yet. The decoder stops at the first unknown register and loses the
...
- **`0x44`.** Sits between 136 and 141, about two units higher in winter than in
  summer, is exactly constant within a day, does not react to the fan setpoint,
  and climbs linearly from 10 to its final value over roughly 135 seconds after
  power-up. The ramp suggests an actuator, the independence from the setpoint
  rules out anything air related. Until that is settled it is exported raw as
  `climos_register{register="0x44"}`.
- **`0x85`.** Runs exactly parallel to `0x26`, 20 d 13:37 behind. Fan hours
  against device hours would explain the offset, but nothing proves it.
...
- **Which register reports the preheater.** That needs a recording from a day
  below −2 °C; the first such day after 2026-02-21 is still to come.
